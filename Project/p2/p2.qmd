...
# Feature selection using Random Forest with reduced number of trees
set.seed(42)
feature_selector <- randomForest(
    x = X_train,
    y = y_train,
    ntree = 50, # Reduced number of trees for quicker computation
    mtry = floor(sqrt(ncol(X_train))), # Default value, can be reduced further
    nodesize = 10, # Minimum size of terminal nodes
    maxnodes = 30, # Maximum number of terminal nodes
    importance = TRUE,
    do.trace = 10
)
...
```{r ml2}
# Function to extract performance metrics from a confusion matrix
...
# Calculate performance metrics and create plots for each model
# Logistic Regression
cm_lr <- confusionMatrix(predictions_lr, y_test)
metrics_lr <- extract_metrics(cm_lr)
plot_cm_lr <- plot_confusion_matrix(cm_lr, "Logistic Regression Confusion Matrix")

# Random Forest
cm_rf <- confusionMatrix(predictions_rf, y_test)
metrics_rf <- extract_metrics(cm_rf)
plot_cm_rf <- plot_confusion_matrix(cm_rf, "Random Forest Confusion Matrix")

# Gradient Boosting
cm_gb <- confusionMatrix(factor(predictions_gb), factor(y_test))
metrics_gb <- extract_metrics(cm_gb)
plot_cm_gb <- plot_confusion_matrix(cm_gb, "Gradient Boosting Confusion Matrix")

# Combine performance metrics into one dataframe
combined_metrics <- rbind(metrics_lr, metrics_rf, metrics_gb)
row.names(combined_metrics) <- c("Logistic Regression", "Random Forest", "Gradient Boosting")

# Bar plot of accuracy, sensitivity, and specificity for each model
ggplot(melt(combined_metrics), aes(x = Var2, y = value, fill = Var2)) +
